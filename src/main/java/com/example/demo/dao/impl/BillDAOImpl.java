...
public class BillDAOImpl implements BillDAO {
    ...
    private List<Bill_Item> getBillItems(int billId) throws Exception {
    ...
    private void saveBillItems(int billId, List<Bill_Item> billItems) throws Exception {
        String sql = "INSERT INTO bill_items (bill_id, item_id, quantity, price_at_time) VALUES (?, ?, ?, ?)";
        try (Connection conn = DatabaseConfig.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            for (Bill_Item billItem : billItems) {
                stmt.setInt(1, billId);
